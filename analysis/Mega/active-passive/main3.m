...
%clearvars -except actnorm_faNmiss passnorm
close all
rng('default')
...
%% Checks
if ~exist('actnorm_faNmiss','var') || ~exist('passnorm','var')
    error('Please load actnorm_faNmiss and passnorm structs first.');
end
if ~isfield(actnorm_faNmiss,'Out12') || ~isfield(passnorm,'Out12') ...
   || ~isfield(actnorm_faNmiss,'Out34') || ~isfield(passnorm,'Out34')
...
for s = 1:numel(stimNames)
    for v = 1:numel(varNames)
        ...
        if s <= 2
            ...
            A = actnorm_faNmiss.Out12; P = passnorm.Out12;
            stimField = stim;  % same name
        else
            % stim3, stim4 from Out34 but stored as stim1, stim2
            A = actnorm_faNmiss.Out34; P = passnorm.Out34;
...
function [bootMean, ciLow, ciHigh] = bootstrapOverSubjects(data, nBoot, alpha)
    ...
    for b = 1:nBoot
        idx = randi(ns, ns, 1);
        mBoot(b,:) = nanmean(data(idx,:), 1);
    end
    bootMean = nanmean(mBoot, 1);
