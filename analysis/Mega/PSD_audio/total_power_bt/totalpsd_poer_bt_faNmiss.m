...
stimFields = fieldnames(PSD_Spect_Struct_actnorm_faNmiss);
channels = fieldnames(PSD_Spect_Struct_actnorm_faNmiss.(stimFields{1})(1)); % e.g. ch_AF3, ch_F7, ...
...
for sIdx = 1:numel(stimFields)
    ...
    for bIdx = 1:length(bands)
        ...
        % -------------------------------------------------------------
        nSubjects = numel(PSD_Spect_Struct_actnorm_faNmiss.(stim));
        ...
        for s = 1:nSubjects
            for c = 1:nChannels
                ...
                activeBandMat(s,c) = PSD_Spect_Struct_actnorm_faNmiss.(stim)(s).(channels{c}).bandPowerMatrix(bIdx);
