function [bootMean, bootCI, freq] = bootstrap_psdVisual(dataStruct, seg, chan, band, nBoot)
    % Collect data across subjects
    nSubjects = numel(dataStruct.(seg));
    allSubjectsData = cell(nSubjects,1);
    for s = 1:nSubjects
        allSubjectsData{s} = dataStruct.(seg)(s).(chan).(band);
    end
    ...
    for b = 1:nBoot
        ...
        bootMeans(:,b) = mean(groupData(:,idx), 2);
    end
    ...
    freq = dataStruct.(seg)(1).(chan).([band '_f']);
end
