function plot_eeg_moving_rms_overlap(actnorm, passnorm, win_samples, overlap_samples, subtract_baseline, base_samples, norm_type)
% plot_eeg_moving_rms_overlap - Overlapping RMS for Normalized data

    if nargin < 3 || isempty(win_samples), win_samples = 10; end
    if nargin < 4 || isempty(overlap_samples), overlap_samples = 5; end 
    if nargin < 5 || isempty(subtract_baseline), subtract_baseline = true; end 
    if nargin < 6 || isempty(base_samples), base_samples = 128; end
    if nargin < 7 || isempty(norm_type), norm_type = 'zscore'; end

    if (win_samples - overlap_samples) <= 0
        error('Overlap samples must be strictly less than window samples.');
    end

    act_12_1 = extract_eeg_rms_overlap(actnorm.Out12, 1, win_samples, overlap_samples, subtract_baseline, base_samples, norm_type);
    act_12_2 = extract_eeg_rms_overlap(actnorm.Out12, 2, win_samples, overlap_samples, subtract_baseline, base_samples, norm_type);
    act_34_1 = extract_eeg_rms_overlap(actnorm.Out34, 1, win_samples, overlap_samples, subtract_baseline, base_samples, norm_type);
    act_34_2 = extract_eeg_rms_overlap(actnorm.Out34, 2, win_samples, overlap_samples, subtract_baseline, base_samples, norm_type);
    
    pass_12_1 = extract_eeg_rms_overlap(passnorm.Out12, 1, win_samples, overlap_samples, subtract_baseline, base_samples, norm_type);
    pass_12_2 = extract_eeg_rms_overlap(passnorm.Out12, 2, win_samples, overlap_samples, subtract_baseline, base_samples, norm_type);
    pass_34_1 = extract_eeg_rms_overlap(passnorm.Out34, 1, win_samples, overlap_samples, subtract_baseline, base_samples, norm_type);
    pass_34_2 = extract_eeg_rms_overlap(passnorm.Out34, 2, win_samples, overlap_samples, subtract_baseline, base_samples, norm_type);
    ...
    figure('Name', 'Norm RMS Overlap: Active & Passive Avg');
    ...
    figure('Name', 'Norm RMS Overlap: Stim12');
    ...
    figure('Name', 'Norm RMS Overlap: Stim34');
    ...
    figure('Name', 'Norm RMS Overlap: REG vs REG');
...
function out_data = extract_eeg_rms_overlap(normOut, stim_idx, win, ovlp, sub_bl, base_samples, norm_type)
    field_name = sprintf('stim%d_%s', stim_idx, norm_type);
    ft = permute(normOut.(field_name), [3, 2, 1]); % [time x ch x subj]
    ...
    % 1. Moving RMS over time per channel
    rms_ft = zeros(num_win, num_ch, num_subj);
    for i = 1:num_win
    ...
    if sub_bl
        ...
        rms_bl = zeros(length(starts_bl), num_ch, num_subj);
        for i = 1:length(starts_bl)
            idx = starts_bl(i):(starts_bl(i) + win - 1);
            rms_bl(i, :, :) = rms(bl(idx, :, :), 1, 'omitnan');
        end
        bl_rms_mean = mean(rms_bl, 1, 'omitnan'); % [1 x ch x subj]
        rms_ft = rms_ft - bl_rms_mean;
    end
    
    % 2. THEN average across channels (dim 2)
    rms_ch_avg = mean(rms_ft, 2, 'omitnan'); % [num_win x 1 x subj]
...
function plot_with_ci(ax, t, data1, name1, data2, name2, plt_title)
    ...
    title(plt_title, 'FontSize', 11); xlabel('Time (s)', 'FontSize', 10); ylabel('Norm Moving RMS Overlap', 'FontSize', 10);
