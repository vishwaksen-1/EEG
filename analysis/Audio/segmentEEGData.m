function db = segmentEEGData(sub, setno, T_data_cleaned, experiment_type)
% segment_eeg_data - Unified function to segment EEG data for active or passive experiments.
%   This function handles the initial data block extraction based on the
%   experiment type before calling the core segmentation function.
%
%   Args:
...
if strcmpi(experiment_type, 'active')
    db{1,4} = segmentBlock(blocksEEG{2}, fs, segDur, setno);
    db{1,5} = blocksEEG{3};
elseif strcmpi(experiment_type, 'passive')
